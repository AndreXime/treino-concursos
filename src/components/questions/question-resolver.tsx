...
import { alternativeOptionClass, feedbackPanelClass } from "@/lib/ui/question-styles"
  ...
  return (
    <article className="rounded-2xl border border-border bg-surface p-5 sm:p-8">
      <div className="mb-5 flex flex-wrap gap-2 text-xs font-medium text-muted">
        <span className="rounded-md bg-background px-2 py-1">Questão {question.numero}</span>
        <span className="rounded-md bg-accent-soft px-2 py-1 text-accent-strong">
          {question.disciplina}
        </span>
        <span className="rounded-md bg-background px-2 py-1">{prova.banca}</span>
