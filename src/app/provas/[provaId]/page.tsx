...
export default async function ProvaPage({ params, searchParams }: ProvaPageProps) {
  const { provaId } = await params
  const { disciplina, initialNumero } = parseProvaSearchParams(await searchParams)
    ...
    <div className="space-y-6">
      ...
      {questions.length === 0 ? (
        <div className="rounded-xl border border-dashed border-border bg-surface/70 px-5 py-10 text-center">
          <p className="text-foreground">Nenhuma questão encontrada com esse filtro.</p>
          <Link
