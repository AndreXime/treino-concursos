...
export class InMemoryProvaRepository implements ProvaRepository {
  ...
  async listQuestoes(provaId: string, filters?: QuestionFilters): Promise<Question[]> {
    const prova = await this.getProvaById(provaId)
    if (!prova) {
      return []
    }
    return prova.questoes.filter((question) => matchesFilters(question, filters))
  }

  async getQuestao(provaId: string, questionId: string): Promise<Question | null> {
    const prova = await this.getProvaById(provaId)
    if (!prova) {
      return null
    }
    return prova.questoes.find((question) => question.id === questionId) ?? null
  }
  ...
  async getFilterOptions(provaId: string): Promise<QuestionFilterOptions> {
    ...
    const disciplinas = [...new Set(prova.questoes.map((q) => q.disciplina))].sort()
    return { disciplinas }
  }
  ...
  async getNextQuestionId(
    ...
    const currentIndex = filtered.findIndex((question) => question.id === currentQuestionId)
